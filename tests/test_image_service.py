...
async def test_generate_returns_image(image_service):
...
async def test_prompt_enrichment_pictogram(image_service):
...
async def test_prompt_enrichment_realistic(image_service):
    req = ImageGenerationRequest(prompt="cat", style="realistic")
    enriched = image_service._build_prompt(req)
    assert "photorealistic" in enriched.prompt.lower()


async def test_unknown_style_passes_through(image_service):
